...
app.get("/getAllNotes", express.json(), async(req, res) => {
    try {
        ...
        //getting username
        const request_username = decoded.username;
        console.log(request_username);
        const query = {username : request_username};
        ...
        const output_list = await cursor.toArray();

        return res.status(200).json({response : output_list})
        });
    } catch (error) {
        res.status(500).json({error: error.message});
    }
...
app.delete("/deleteNote/:noteId", express.json(), async(req, res) => {
  try {
      ...
      jwt.verify(token, "secret-key", async (err, decoded) => {
          ...
          const noteId = req.params.noteId;
          if (noteId == "") {
            return res.status(400).send("no noteId");
          }
          ...
          // checking data
          if (!data) {
            return res.status(404).send("Bad noteId");
          }

          if (data.username != decoded.username) {
            return res.status(404).send("The note doesn't belong to you.");
          }
...
app.patch("/editNote/:noteId", express.json(), async(req, res) => {
    try {
      ...
      jwt.verify(token, "secret-key", async (err, decoded) => {
          ...
          const result = await collection.updateOne({_id : new ObjectId(noteId)}, update_document);
